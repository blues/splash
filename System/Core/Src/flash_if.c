    ...
    if (ret_status == FLASH_IF_OK) {
        ...
        if (HAL_FLASH_Unlock() == HAL_OK) {
            ...
            for (page_index = start_page_index; page_index < (start_page_index + number_pages); page_index++) {
                ...
                // (ray) was incorrectly: if (FLASH_IF_INT_IsEmpty(pDestination, length) != FLASH_IF_MEM_EMPTY) {
...
// This function checks if part of Flash is empty
// pStart flash address to check
// uLength number of bytes to check. It has to be 8 bytes aligned.
// returns int32_t FLASH_IF_MEM_EMPTY or FLASH_IF_MEM_NOT_EMPTY
static int32_t FLASH_IF_INT_IsEmpty(void *pStart, uint32_t uLength)
{
