...
int pdm2pcm_init(int bit_order, int endianess, int sinc)
{
    ...
    if (arm_fir_decimate_init_q15(&fir_decim_S, N_TAPS_FIR_DEC, DEC_OUT_FACTOR, fir_taps[volume], FirState, (BLOCK_SIZE / DEC_CIC_FACTOR)) != ARM_MATH_SUCCESS) {
        return -1;
    }
